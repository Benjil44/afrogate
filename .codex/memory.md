...
- Client subscription config rendering is now split into safe readiness descriptors and future secret-backed generation. `/api/client/subscription` can report WireGuard, VLESS, L2TP, and IKEv2 render status, missing public endpoint material, and required per-client secret material, but it must not return private keys, client UUIDs, PSKs, passwords, certificates, raw outbound config JSON, or connectable secret-bearing links until an audited encrypted per-client renderer exists.
