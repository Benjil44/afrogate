...
- Telegram bot creation must happen in Telegram through BotFather. AfroGate should provide a future superadmin Settings setup wizard that accepts the BotFather token once, stores it encrypted/write-only, captures allowed chat/admin IDs and webhook secret, and tests Telegram API reachability through the shared outbound egress path without echoing the token.
