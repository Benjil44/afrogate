...
- Configured `origin` as `https://github.com/Benjil44/afrows.git`.
...
- User created GitHub repository `Benjil44/afrows`.
- Updated `origin` to `https://github.com/Benjil44/afrows.git`.
...
- Added Python agent support for `AFROWS_OUTBOUND_PROXY_URL`.
...
- Added local scrypt-hashed managed admin user storage through `AFROWS_ADMIN_USERS_FILE`, defaulting to git-ignored `tmp/admin-users.json`.
- Bumped Afrows to `0.6.0` for the admin user-management capability.
...
- Remote target is configured for `Benjil44/afrows` and local `main` tracks `origin/main`.
...
- Bumped Afrows to `0.6.1` for the Users page UI refinement.
...
- Bumped Afrows to `0.6.2` for the follow-up Users and Servers page layout refinement.
...
- Bumped Afrows to `0.6.3` for the protocol-aware smart-routing documentation update.
...
- Bumped Afrows to `0.7.0` for the server edit workflow capability.
...
- Bumped Afrows to `0.8.0` for the agent registration capability.
...
- Bumped Afrows to `0.9.0` for the server agent heartbeat capability.
- Standardized direct local development ports to dashboard `3000` and backend `8000`.
- Added strict Vite port behavior so Afrows does not drift to `3001+` when port `3000` is busy.
...
- Bumped Afrows to `0.9.1` for local development and Playwright test wiring.
...
- Added `scripts/setup-local-postgres.ps1` and root `db:setup:local` to install/prepare local PostgreSQL, create the `afrows` role/database, optionally write `.env`, and run migrations.
- Documented local PostgreSQL setup in `docs/local-development.md` and `infra/postgres/README.md`.
- Bumped Afrows to `0.9.2` for local PostgreSQL setup tooling and documentation.
- Installed PostgreSQL 18.4 locally on Windows, created the `afrows` role/database, wrote the local ignored `.env`, and applied all PostgreSQL migrations.
...
- Bumped Afrows to `0.10.0` for the guarded alert API and dashboard alert binding capability.
...
- Moved Afrows direct local development wiring to dashboard `4000` and backend `7000` so ports `3000` and `8080` can be used by another app.
...
- Bumped Afrows to `0.11.0` for real Servers/Routes API binding.
...
- Bumped Afrows to `0.14.0` for the alert engine capability.
...
- Bumped Afrows to `0.13.0` for the outbound health scheduler capability.
...
- Bumped Afrows to `0.12.0` for the Telegram critical-alert delivery foundation.
...
- Bumped Afrows to `0.15.0` for the Settings/protocol setup workflow.
...
- Bumped Afrows to `0.16.0` for the Settings persistence/API foundation.
...
- Added backend `SecretVaultService` using AES-256-GCM with `AFROWS_SECRETS_KEY` and `AFROWS_SECRETS_KEY_ID`.
...
- Documented the local/deployment requirement for `AFROWS_SECRETS_KEY`.
- Marked the encrypted Settings private-key persistence checklist item complete.
- Bumped Afrows to `0.17.0` for the encrypted Settings secret-storage capability.
...
- Bumped Afrows to `0.18.0` for the initial protocol provisioning capability.
...
- Applied PostgreSQL migrations through `0005_secret_records.sql` with `npm.cmd --workspace @afrows/backend run db:migrate`.
...
- Applied PostgreSQL migrations through `0006_protocol_provisioning.sql` with `npm.cmd --workspace @afrows/backend run db:migrate`.
...
- Verified a one-shot local agent run with `AFROWS_PING_TARGETS=127.0.0.1` returns populated `pingMs`, `jitterMs`, and `packetLossPercent` values without sending data to the backend.
...
- Bumped Afrows to `0.23.0` for the advisory protocol-aware route scoring capability.
...
- Bumped Afrows to `0.24.0` for the initial route-quality analytics and recommendation capability.
...
- Bumped Afrows to `0.25.0` for the hourly route-quality aggregation foundation.
...
- Bumped Afrows to `0.26.0` for dimensioned route intelligence and predictive advisory recommendations.
...
- Bumped Afrows to `0.29.0` for persisted route assignment controls.
...
- Bumped Afrows to `0.33.0` for the route apply plan foundation.
- Added route apply adapter readiness metadata to preview apply plans for the future WireGuard policy-routing adapter.
- Added disabled-by-default `AFROWS_ROUTE_DATA_PLANE_APPLY_ENABLED=false` configuration and surfaced the adapter/feature-flag state in Settings.
- Kept adapter implementation marked missing and `dataPlaneReady=false` until the real server-side apply adapter is audited.
- Bumped Afrows to `0.34.0` for the route apply adapter readiness layer.
...
- Bumped Afrows to `0.35.0` for the dry-run WireGuard apply preview.
...
- Bumped Afrows to `0.36.0` for persisted dry-run apply snapshots in route decision events.
...
- Bumped Afrows to `0.37.0` for route decision event detail inspection.
...
- Verified a one-shot local agent run with `AFROWS_DNS_PROBE_TARGETS=localhost` returns a healthy `routeProbes` DNS row without sending data to the backend.
...
- Applied PostgreSQL migrations through `0007_route_quality_hourly.sql` with `npm --workspace @afrows/backend run db:migrate`.
...
- Applied PostgreSQL migrations through `0008_route_quality_dimensions.sql` with `npm --workspace @afrows/backend run db:migrate`.
- Verified workspace TypeScript checks with `npm run typecheck --workspaces --if-present`.
- Verified production build with `npm run build --workspaces --if-present`.
...
- Verified fixed-port dashboard smoke test with `npm run test:e2e`.
- Verified whitespace safety with `git diff --check`; only existing CRLF conversion warnings were reported.
- Applied PostgreSQL migrations through `0009_route_decision_foundation.sql` with `npm --workspace @afrows/backend run db:migrate`.
...
- Verified fixed-port dashboard smoke test with `npm run test:e2e`.
- Verified whitespace safety with `git diff --check`; only existing CRLF conversion warnings were reported.
- Applied PostgreSQL migrations through `0010_protocol_target_server.sql` with `npm --workspace @afrows/backend run db:migrate`.
...
- Bumped Afrows to `0.84.0` for the route usage multiplier and client subscription refresh foundation.
...
- Verified focused shared, backend, and client typing with `npm run typecheck --workspace @afrows/shared`, `npm run typecheck --workspace @afrows/backend`, and `npm run typecheck --workspace @afrows/client`.
- Verified version alignment with `npm run version:check`.
- Verified repository secret hygiene with `npm run secrets:check`.
- Verified workspace typecheck and production build with `npm run typecheck` and `npm run build`.
- Verified dashboard contrast with `npm run contrast:check`.
...
- Applied PostgreSQL migrations through `0011_protocol_apply_events.sql` with `npm --workspace @afrows/backend run db:migrate`.
...
- Bumped Afrows to `0.57.0` for tunnel/interface inventory management.
- Applied PostgreSQL migrations through `0012_tunnels_interfaces.sql` with `npm --workspace @afrows/backend run db:migrate`.
...
- Bumped Afrows to `0.61.0` for the CI and secret-scan workflow.
...
- Hardened the backend systemd sample around `/etc/afrows/afrows.env`, dedicated service state/log directories, localhost binding, and low-privilege runtime settings.
...
- Bumped Afrows to `0.61.1` for the Ubuntu deployment documentation and samples.
...
- Bumped Afrows to `0.61.2` for the optional Docker Compose deployment samples.
...
- Bumped Afrows to `0.62.0` for the completed protocol-aware agent route-probe capability.
...
- Bumped Afrows to `0.62.1` for the shared dashboard panel-state UI refinement.
...
- Bumped Afrows to `0.62.2` for the mobile/tablet resource strip density refinement.
...
- Bumped Afrows to `0.62.3` for the dashboard tooltip refinement.
...
- Bumped Afrows to `0.62.4` for the dashboard contrast guard.
...
- Bumped Afrows to `0.62.5` for the dashboard visual capture tests.
...
- Bumped Afrows to `0.63.0` for the Alerts page filters/history capability.
...
- Updated protocol apply preflight reason codes, env samples, deployment notes, memory, checklist, and server-access/security docs for `AFROWS_PROTOCOL_SERVER_APPLY_CREDENTIAL_DECRYPT_ENABLED=false`.
...
- Bumped Afrows to `0.63.1` for the protocol apply credential-decrypt readiness gate.
...
- Updated protocol apply preflight reason codes, env samples, deployment notes, memory, checklist, and server-access/security docs for `AFROWS_PROTOCOL_SERVER_APPLY_SECRET_DECRYPT_ENABLED=false`.
- Verified protocol apply secret-decrypt typing with `npm run typecheck --workspaces --if-present`.
- Bumped Afrows to `0.63.2` for the protocol apply secret-decrypt readiness gate.
- Verified version alignment with `npm run version:check`.
- Verified repository secret hygiene with `npm run secrets:check`.
- Verified dashboard contrast with `npm run contrast:check`.
...
- Bumped Afrows to `0.77.0` for the rewarded-ad admin settings API contract.
...
- Verified focused shared/backend typing with `npm run typecheck --workspace @afrows/shared` and `npm run typecheck --workspace @afrows/backend`.
- Verified version alignment with `npm run version:check`.
- Verified repository secret hygiene with `npm run secrets:check`.
- Verified dashboard contrast with `npm run contrast:check`.
- Verified workspace typecheck and production build with `npm run typecheck --workspaces --if-present` and `npm run build --workspaces --if-present`.
...
- PayPal credentials, webhook ID, API base URL, return/cancel URLs, and timeout are configured through `AFROWS_PAYPAL_*` deployment environment values; provider secrets remain out of `payment_methods.public_config`.
- PayPal outbound calls use the shared proxy-aware backend outbound HTTP client so restricted deployments can reuse `AFROWS_OUTBOUND_PROXY_URL`.
...
- Bumped Afrows to `0.74.0` for the PayPal adapter API contract.
- Checklist completion after this slice is `199 / 222` items, or `89.6%` complete with `10.4%` remaining.
- Verified shared/backend typing with `npm run typecheck --workspace @afrows/shared` and `npm run typecheck --workspace @afrows/backend`.
- Verified version alignment with `npm run version:check`.
- Verified repository secret hygiene with `npm run secrets:check`.
- Verified workspace typecheck and production build with `npm run typecheck --workspaces --if-present` and `npm run build --workspaces --if-present`.
- Verified dashboard contrast with `npm run contrast:check`.
...
- Bumped Afrows to `0.75.0` for the allocation schema/API contract.
- Checklist completion after this slice is `201 / 223` items, or `90.1%` complete with `9.9%` remaining.
- Verified migration `0019_payment_order_allocations.sql` with `npm --workspace @afrows/backend run db:migrate`.
...
- Verified focused shared/backend/client typing with `npm run typecheck --workspace @afrows/shared`, `npm run typecheck --workspace @afrows/backend`, and `npm run typecheck --workspace @afrows/client`.
- Verified migration `0020_rewarded_ad_grants.sql` with `npm --workspace @afrows/backend run db:migrate`.
- Verified version alignment with `npm run version:check`.
- Verified repository secret hygiene with `npm run secrets:check`.
- Verified dashboard contrast with `npm run contrast:check`.
- Verified workspace typecheck and production build with `npm run typecheck --workspaces --if-present` and `npm run build --workspaces --if-present`.
...
- Bumped Afrows to `0.71.0` for the client-scoped mobile API/auth contract.
- Checklist completion after this slice is `195 / 222` items, or `87.8%` complete with `12.2%` remaining.
- Verified the client access token migration with `npm --workspace @afrows/backend run db:migrate`.
...
- Verified repository secret hygiene with `npm run secrets:check`.
- Verified dashboard contrast with `npm run contrast:check`.
- Verified the focused shared/backend typing with `npm run typecheck --workspace @afrows/shared` and `npm run typecheck --workspace @afrows/backend`.
- Verified workspace typecheck and production build with `npm run typecheck --workspaces --if-present` and `npm run build --workspaces --if-present`.
...
- Verified focused client typing and production build with `npm run typecheck --workspace @afrows/client` and `npm run build --workspace @afrows/client`.
...
- Bumped Afrows to `0.73.0` for the usage-accounting API and schema contract.
- Checklist completion after this slice is `198 / 222` items, or `89.2%` complete with `10.8%` remaining.
- Verified shared/backend typing with `npm run typecheck --workspace @afrows/shared` and `npm run typecheck --workspace @afrows/backend`.
- Verified migration `0018_client_usage_events.sql` with `npm --workspace @afrows/backend run db:migrate`.
...
- Bumped Afrows to `0.69.0` for the client route preference schema/API contract.
- Checklist completion after this slice is `193 / 222` items, or `86.9%` complete with `13.1%` remaining.
- Verified the client route preference migration with `npm --workspace @afrows/backend run db:migrate`.
...
- Bumped Afrows to `0.68.0` for the payment-order lifecycle schema/API contract.
- Checklist completion after this slice is `191 / 217` items, or `88.0%` complete with `12.0%` remaining.
- Verified the payment-order migration with `npm --workspace @afrows/backend run db:migrate`.
...
- Bumped Afrows to `0.67.0` for the billing package/payment-method API and schema contract.
...
- Verified the billing catalog foundation with `npm --workspace @afrows/backend run db:migrate`.
...
- Bumped Afrows to `0.64.0` for the protocol apply config-material readiness contract.
- Verified version alignment with `npm run version:check`.
- Verified repository secret hygiene with `npm run secrets:check`.
- Verified dashboard contrast with `npm run contrast:check`.
- Verified workspace typecheck and production build with `npm run typecheck --workspaces --if-present` and `npm run build --workspaces --if-present`.
...
- Bumped Afrows to `0.65.0` for the protocol apply command-policy readiness contract and account quota model decision.
...
- Added write-only paid-number handling that stores only HMAC hashes using `AFROWS_IDENTITY_HASH_KEY` or the deployment secrets key fallback.
...
- Bumped Afrows to `0.66.0` for the customer-account quota-management foundation.
- Checklist completion after this slice is `187 / 213` items, or `87.8%` complete with `12.2%` remaining.
- Verified the customer-account foundation with `npm --workspace @afrows/backend run db:migrate`.
...
- Verified the migration stack with `npm --workspace @afrows/backend run db:migrate`.
- Verified focused dashboard/shared typing with `npm run typecheck --workspace @afrows/dashboard` and `npm run typecheck --workspace @afrows/shared`.
- Verified version alignment with `npm run version:check`.
- Verified repository secret hygiene with `npm run secrets:check`.
- Verified dashboard contrast with `npm run contrast:check`.
- Verified workspace typecheck and production build with `npm run typecheck --workspaces --if-present` and `npm run build --workspaces --if-present`.
...
- Verified the migration stack with `npm --workspace @afrows/backend run db:migrate`.
- Verified focused dashboard typing with `npm run typecheck --workspace @afrows/dashboard`.
- Verified version alignment with `npm run version:check`.
- Verified repository secret hygiene with `npm run secrets:check`.
- Verified dashboard contrast with `npm run contrast:check`.
- Verified workspace typecheck and production build with `npm run typecheck --workspaces --if-present` and `npm run build --workspaces --if-present`.
- Verified the final dashboard production build with `npm run build --workspace @afrows/dashboard`.
...
- The webhook is disabled by default, requires `AFROWS_TELEGRAM_BOT_COMMANDS_ENABLED=true`, a configured bot token, and a matching Telegram secret-token header before it processes updates.
...
- Bumped Afrows to `0.80.0` for the Telegram user-command workflow.
...
- Added deployment controls for `AFROWS_RATE_LIMIT_ENABLED`, `AFROWS_RATE_LIMIT_TRUST_PROXY_HEADERS`, and `AFROWS_RATE_LIMIT_MAX_KEYS`.
...
- Bumped Afrows to `0.81.0` for the API-layer rate-limiting foundation.
...
- Ran `npm --workspace @afrows/backend run db:migrate`.
...
- Ran `npm run typecheck --workspace @afrows/backend --workspace @afrows/shared`.
- Ran `npm --workspace @afrows/backend run db:migrate`.
- Ran `npm run version:check`.
- Ran `npm run secrets:check`.
- Ran `npm audit --audit-level=moderate`; zero vulnerabilities found.
...
- Production protocol apply, panel migration adapters, additional payment providers, verified rewarded-ad provider callbacks, reports, tenant branding, and native per-app VPN split tunneling remain future work.
- Existing single-role PostgreSQL deployments should back up first and may need manual object ownership reassignment before future DDL-heavy migrations run as `afrows_migrator`.
...
- Bumped Afrows to `0.92.0` and updated `CHANGELOG.md`.
...
- Bumped Afrows to `0.93.0` and updated `CHANGELOG.md`.
...
- Bumped Afrows to `0.95.0` and updated `CHANGELOG.md`.
...
- Bumped Afrows to `0.96.0` and updated `CHANGELOG.md`.
...
- The export endpoint returns Afrows client config summaries in `afrows_client_configs_export_v1` format, audits compact export metadata, and excludes subscription credentials, secret-bearing config material, raw panel payloads, paid numbers, client tokens, and external-panel API calls.
...
- Bumped Afrows to `0.97.0` and updated `CHANGELOG.md`.
...
- Added guarded `POST /api/admin/current-panels/charge-volume` for audited local Afrows quota top-ups.
...
- Bumped Afrows to `0.98.0` and updated `CHANGELOG.md`.

### Verification

- Ran `npm --workspace @afrows/backend run db:migrate`; local migrations completed through `0025_quota_charge_events.sql`.
- Ran `npm run version:check`.
- Ran `npm run typecheck`.
- Ran `npm run build --workspaces --if-present`.
...
- Bumped Afrows to `0.99.0` and updated `CHANGELOG.md`.
...
- Bumped Afrows to `0.100.0` and updated `CHANGELOG.md`.
...
- Bumped Afrows to `0.101.0` and updated `CHANGELOG.md`.

### Verification

- Ran `npm --workspace @afrows/backend run db:migrate`; local migrations completed through `0026_tenant_brand_settings.sql`.
- Ran `npm run version:check`.
- Ran `npm run typecheck`.
- Ran `npm run build --workspaces --if-present`.
...
- Bumped Afrows to `0.101.1` and updated `CHANGELOG.md`.
...
- Bumped Afrows to `0.102.0` and updated `CHANGELOG.md`.
...
- Added HMAC-SHA256 signature verification over `timestamp.canonicalJson(payload)` using `AFROWS_REWARDED_AD_WEBHOOK_SECRET` plus timestamp freshness through `AFROWS_REWARDED_AD_WEBHOOK_TOLERANCE_SECONDS`.
...
- Bumped Afrows to `0.103.0` and updated `CHANGELOG.md`.
...
- Kiosk mode stores local browser state in `afrows.dashboard.kiosk`, requests browser fullscreen when available, hides the sidebar, and expands the dashboard grid to the full viewport.
...
- Bumped Afrows to `0.103.1` and updated `CHANGELOG.md`.
...
- Bumped Afrows to `0.103.2` and updated `CHANGELOG.md`.
...
- Bumped Afrows to `0.103.3` and updated `CHANGELOG.md`.
...
- Bumped Afrows to `0.104.0` and updated `CHANGELOG.md`.
...
- Ran `npm --workspace @afrows/backend run db:migrate`; all migrations through `0027_admin_users.sql` applied successfully.
- Ran `npm run version:check`.
- Ran `npm run typecheck`.
- Ran `npm run build --workspaces --if-present`.
...
- Bumped Afrows to `0.108.1` and updated `CHANGELOG.md`.
...
- Bumped Afrows to `0.109.0` and updated `CHANGELOG.md`.

### Verification

- Ran `npm --workspace @afrows/backend run db:migrate`; all migrations through `0028_reseller_wallets.sql` applied successfully.
- Ran `npm run version:check`.
- Ran `npm run typecheck`.
- Ran `npm run build --workspaces --if-present`.
- Ran full `npm run test:e2e`; 17 tests passed.
...
- Added a reseller workspace panel with wallet balance, available balance, reseller/Afrows share, and localized wallet ledger labels.
...
- Bumped Afrows to `0.110.0` and updated `CHANGELOG.md`.
...
- Sales can renew an existing owned customer or create a new reseller-owned customer, then credit package quota, record a paid `reseller_wallet` payment order/allocation, and debit Afrows's share from the reseller wallet in one transaction.
...
- Bumped Afrows to `0.111.0` and updated `CHANGELOG.md`.
...
- Added opt-in agent DF/path-MTU diagnostics through `AFROWS_MTU_PROBE_TARGETS` plus bounded MTU range, tunnel-overhead, and configured-MTU environment settings.
...
- Bumped Afrows to `0.112.0` and updated `CHANGELOG.md`.
...
- Added a reseller Dashboard backed only by `/api/admin/reseller/workspace`, with seller sales summary cards, ECharts sales trend, customer quota/service-experience chart, recent customers, and estimated seller margin/Afrows share metrics.
...
- Added a reseller Billing selling summary so representatives can see sales amount, sold volume, Afrows debit, estimated seller margin, orders, and active customers beside the existing wallet and package-sale workflow.
...
- Bumped Afrows to `0.113.0` and updated `CHANGELOG.md`.
...
## 2026-05-30 Reseller Users Add-User Action
...
- Main checklist remains `245 / 245` complete, or `100.0%` complete with `0` unchecked items.
- Bumped Afrows to `0.113.2` and updated `CHANGELOG.md`.
...
- **Deployed Afrows live to the operator's VPS** (`94.74.145.199`, Ubuntu 24.04 LTS, 4 GB RAM, IP-only). Live at `https://94.74.145.199/` with self-signed TLS. Done over a heavily filtered Irelandian network:
  ...
  - Secrets generated on-box in `/etc/afrows/.secrets`; systemd `afrows-backend` unit (127.0.0.1:7000); Nginx self-signed TLS reverse proxy on 443; UFW (22/80/443 allow, default deny).
  - Deploy/ops scripts kept **gitignored** (server-specific, generate secrets on-box): `deploy-afrows.sh`, `update-afrows.sh`, `sync.ps1`, `ufw-afrows.sh`.
- **Passwordless dev→VPS deploy loop.** `sync.ps1` (PC) packages source (excludes node_modules/.git/secrets), ships over SSH (ed25519 `afrows_deploy` key), and runs `update-afrows.sh` on the box: extract → build with `VITE_API_BASE_URL=/api` → idempotent migrate → restart → health check (retries up to 20s). `.\sync.ps1` for code-only, `.\sync.ps1 -WithDeps` re-warms + ships the Linux npm cache when `package-lock.json` changed.
...
- `npm --workspace @afrows/dashboard run build` and `npm --workspace @afrows/backend run build` pass locally (auto-reload wiring compiles clean).
...
- **Live deploy via `sync.ps1` succeeded (exit 0):** VPS rebuilt all workspaces, re-applied all 28 migrations idempotently, `nginx -t` OK + reloaded, and the backend health check returned `{"status":"ok","service":"afrows-backend","version":"0.114.27",...}` — confirming the new version is live and the auto-reload backend half works. Open dashboard tabs still on `0.114.26` will now detect `0.114.27` and reload themselves.
...
### 2026-06-06 Outbounds speed test + scheduler (DONE, T6/T7) — backend-on-box
...
- **Repoint the box's dead internet uplink** (`/usr/local/etc/xray/config.json`, `91.243.114.71:48724`) to a reachable outbound (`185.252.28.28` or `46.245.95.155`) to restore the box's worldwide internet — pending operator go-ahead.
...
### 2026-07-24 Parallel agent-team fixes: 4 operator issues (built + verified, NOT yet committed)

- **Agent team introduced.** Added `.claude/agents/` role specs (cto-architect, scrum-master, senior-frontend-designer [Fable 5], senior-backend-engineer [Opus], network-infra-engineer [Opus], qa-tester) + `README.md`. Model policy: Fable 5 for design/"improve & fix" UI, Opus for daily engineering. See `docs/afrows-fix-roadmap.md` for the 4-issue diagnosis matrix.
- **① Outbound dies on power loss — diagnosed physical, not code.** Every clean exit (Starlink + 3 LTE modems + wg-germany) traverses the village MikroTik on one un-UPS'd circuit; redundancy protects one leg failing, not the shared power feed. **Fix is hardware:** small UPS on the MikroTik + primary LTE modem (keeps `via-germany` alive). In-repo hardening applied to `xray.service`: `Restart=always`, `StartLimitIntervalSec=0` ([Unit]), `network-online.target` ordering (untracked/gitignored deploy file — apply on the box).
- **② Mobile customer table unusable.** `CustomersPage` DataTable is 900px-wide in an overflow-x scroller with Edit/Config in the far-right `alignRight` column → off-screen/untappable on ~390px. **Fix:** extended the shared `DataTable` primitive (`primitives.tsx`, +73/−6 real; rest is CRLF reformat) with optional `renderDetail(row)` + chevron toggle + full-width `colSpan` detail row (pattern from `OutboundsPage`), sticky at scroll-origin so the panel is fully visible at 390px. `CustomersPage` now renders Edit/Config + key fields inline under each row. New i18n keys (colExpiry/colLastConnected/colCost/colTags/expired/detailExpand/detailCollapse) in en+fa. Dashboard typecheck clean.
- **③ 20 GB plan → ~26 GB usable (Phase 4 quota).** Compound bug: `quota-math.ts` `BYTES_PER_GB=1024**3` treated GB as GiB (+7.4%) AND purely-polled enforcement (60s) let fast links overshoot. **Decision (operator): plans are DECIMAL GB (10⁹).** Fixed `BYTES_PER_GB=1_000_000_000`; audited all uses in backend+shared (billing.service round-trips via the constant; rewarded-ad cap + DTO fixed to decimal). Cut poll windows: Xray 60s→15s (clamp [10,3600]), WG 30s→15s (floor 10s), env-overridable, with `TODO(quota-cap)` for a future inline data-plane cap. New quota-math test proves gbToBytes(20)===20e9 and the `>=` cutoff trips at 20e9 not 19_999_999_999. Backend typecheck clean; quota+rewarded-ad tests pass (2 pre-existing command-safety "Ireland" casing failures are unrelated).
- **④ Weather won't open over VLESS.** `smart` mode's `geoip:ir → direct` + `domainStrategy: IPIfNonMatch` + no DNS block → weather CDNs resolved by the filtered domestic resolver into `geoip:ir` → out the censored `direct` uplink. **Fix:** `afrows-egress-mode-sync.py` now writes a trusted `dns` block (1.1.1.1 DoH / 8.8.8.8 / dns.google, `queryStrategy: UseIP`), folded into the idempotency gate. Kept `IPIfNonMatch` (rejected `AsIs` — would leak domestic `.ir` traffic out the foreign gateway). `py_compile` clean.
- **Incident (recovered):** the backend agent ran `git stash` to test on clean HEAD, sweeping other agents' uncommitted work; `auth.ts` (dev login-skip) + `afrows-egress-mode-sync.py` were left displaced in `stash@{0}` and restored via targeted `git checkout stash@{0} -- <file>`. **Lesson: parallel agents must never `git stash` a shared working tree.** `stash@{0}` retained as a safety net until operator confirms.
