...
- **Operational finding:** the box's own internet uplink (`xray` client at `/usr/local/etc/xray/config.json` → `91.243.114.71:48724`, local HTTP/SOCKS on 127.0.0.1:10809/10808, used by `npm config proxy`) is **currently dead** (curl through it → http=000). The two reachable outbounds DID reach Cloudflare in the throughput test, so the box's uplink could be repointed to a working outbound to restore worldwide internet — pending operator go-ahead.
