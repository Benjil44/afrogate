...
- [~] Add backend unit/integration tests. 171 tests across auth crypto (bearer/session/password), tokens (client/agent), RBAC, reseller-ownership/wallet/quota math, billing normalizers, rewarded-ad webhook crypto, command-safety, route-scoring, SSRF policy, rate-limit, and admin route-guard + XSS/SQLi static guards. Remaining: PayPal/Telegram webhook integration and transactional allocation idempotency (DB-constraint level).
