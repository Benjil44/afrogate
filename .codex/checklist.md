...
- [~] Add backend unit/integration tests. 74 tests across `bearer-token`, `rbac`, `password`, `session-token`, `reseller-ownership`, `reseller-wallet-math`, `client-token`. Established the service-test pattern: extract executor-parameterized logic into non-decorated modules and drive it with an in-memory fake `DatabaseQueryExecutor`. Still pending: payment provider/webhook verification and transactional allocation idempotency.
