...
- [~] Add backend unit/integration tests. 187 tests across auth crypto, tokens, RBAC, reseller/wallet/quota math, billing normalizers, webhook verification (rewarded-ad/PayPal/Telegram), command-safety, route-scoring, SSRF, rate-limit, and static guards. Remaining: transactional allocation idempotency (DB-constraint level). Remaining: PayPal/Telegram webhook integration and transactional allocation idempotency (DB-constraint level).
