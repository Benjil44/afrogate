...
- [x] **Public landing site `apps/web` (DONE 2026-06-05).** New Vite+React 19+Tailwind v4 app live at `afrows.com`/`www`; dark cinematic, bilingual FA/EN (default **EN**, RTL for FA), GSAP + framer-motion. Home: blinking **50%-off promo bar**, parallax hero (animated-gradient headline, floating orbs, trust badges), animated metrics, motion feature cards, **advanced Gamers section** (animated latency graph + ping-comparison bars, neon), audience split, static pricing, cinematic footer (rebranded). Login/Get-started → `app.afrows.com` (centralized `APP_URL`). Plan: `docs/superpowers/plans/2026-06-05-afrows-landing-website.md`.
...
- [x] **New "Outbounds" dashboard page (DONE 2026-06-06).** Live at `app.afrows.com` → Outbounds: table (Name/Type/Status/Ping/Jitter/Download/Upload/Actions, ~20s poll), Add panel (protocol picker VLESS/WireGuard/L2TP + paste-`vless://` auto-fill via backend `outbound-vless-parser` import, monitor-only self-contained config — **not** wired to subscription rendering), per-row Test/Enable-Disable/Delete, **Sync now**, **Auto toggle**. Reuses outbound CRUD + agent ping/jitter/status probes; `listOutbounds` enriched with latest latency/jitter (from `outbound_health_checks`) + throughput. New shared types, migration `0029_outbound_throughput.sql`, endpoints `POST /outbounds/:id/test` + `/outbounds/test-all` + `outbound-test-settings` GET/PATCH. Verified on box (endpoints 200, migration applied, 374/374 backend tests). Also fixed a latent XSS-guard failure from the login redesign (moved inline `<style>` to the stylesheet).
- [x] **On-demand testing — DONE.** Test/Sync-now/Auto queue requests via `speed_test_requested_at`; the backend speed-test engine fulfils them and fills Status/Ping/Jitter/Download/Upload.
- [x] **Speed test + auto scheduler (DONE 2026-06-06, T6/T7).** Implemented **backend-on-box** (not in the Python agent — the agent is static/env-driven with no command channel; backend + Postgres + `xray` share one host). New `outbound-speed-test.service.ts`: a timer that (T7) flags enabled outbounds for re-test when Auto is on and the interval elapsed, and (T6) picks up `speed_test_requested_at`, measures TCP latency/jitter to `config.address`, and measures download/upload throughput by spawning a throwaway `xray` SOCKS proxy bound to the outbound and transferring a fixed payload (curl, default Cloudflare `__down`/`__up`) through it. Results land in `outbound_health_checks` (ping/jitter) + `outbounds.latest_down_mbps/up_mbps`; request flag always cleared. Pure xray-config builder (`outbound-xray-config.ts`, tcp/ws/grpc + none/tls/reality + http header) is unit-tested. Verified live on box: 3 real VLESS outbounds tested — 2 healthy with real Mbps, 1 (`benjil`@91.243.114.71) correctly **critical/unreachable from the VPS** (datacenter-network filtering; reachable only from the home/Starlink vantage where v2ray runs). Also fixed `outbound-health.service.ts` to dial `config.address` instead of the obfuscation `config.host` (was the "Down/5003ms timeout" bug). 378/378 backend tests.
