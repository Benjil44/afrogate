...
- [~] **On-demand testing — queue side done; speed measurement deferred.** Test/Sync-now/Auto are wired and queue requests (`speed_test_requested_at`); **Download/Upload show `—` until the agent speed test lands** (below). Live Status/Ping/Jitter already populate from existing agent probes.
- [ ] **Agent speed test (DEFERRED — develop against the live box).** `apps/agent/collect.py` download/upload throughput routed through the outbound's local proxy + backend ingest into `latest_down_mbps`/`latest_up_mbps` + 10-min auto scheduler. Needs the real VLESS proxy to implement/verify; not guessed blind. Plan tasks T6/T7 in `docs/superpowers/plans/2026-06-06-outbounds-management.md`.
