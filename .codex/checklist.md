...
- [ ] **Live VPS production deployment — DEFERRED by operator until the app is 100% complete (decided 2026-06-03); do not start setup until then.** Operator's VPS: **4 GB RAM, IP-only (no domain yet → self-signed TLS until a domain is added)**. Target OS: **Ubuntu 24.04 LTS** (native path per `infra/ubuntu/README.md` + `docs/enterprise-deployment-guide.md`: Node 22, PostgreSQL, Nginx). Sequence when activated: provision → base packages + `afrogate` service user → clone to `/opt/afrogate` + `npm ci` + build → least-privilege DB roles + migrations → `/etc/afrogate/afrogate.env` secrets → systemd unit + Nginx (self-signed TLS) site + UFW → run `scripts/drills/verify-install.sh`. This unblocks the install/backup/rotation drills below (executable for real once deployed).
