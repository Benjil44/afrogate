...
- [~] Add backend unit/integration tests. 125 tests across bearer-token, rbac, password, session-token, client-token, agent-token, reseller-ownership, reseller-wallet-math, quota-math, rewarded-ad-webhook crypto, command-safety, outbound-url-policy (SSRF), rate-limit-window, and the admin route-guard + XSS/SQLi static guards. Remaining: PayPal/Telegram webhook integration and the transactional allocation idempotency check-then-insert (DB-constraint level).
...
- [~] Test wallet/quota math. Reseller wallet math (`reseller-wallet-math.test.ts`) and the allocation quota math (`quota-math.test.ts`: null-limit→used baseline + purchased, existing-limit add, overflow guard, no retroactive forgiveness) are covered. Remaining: the transactional check-then-insert idempotency / no-double-credit, which is enforced by a DB unique constraint inside a transaction.
...
- [~] Auth/JWT tampering: session-token forgery/tamper + payload validation (`session-token.test.ts`), client-token scope enforcement (`client-token.test.ts`), and agent/client token hashing (`agent-token.test.ts`) covered; the Telegram webhook secret uses the tested constant-time `secureTokenEquals`. Remaining: session-expiry rejection exercised through the decorated AuthService guard.
