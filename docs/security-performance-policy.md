# Afrows Security and Performance Policy

## Mission

Afrows must be designed like infrastructure people depend on, not a casual dashboard. Stable, private, high-quality internet access can affect work, safety, education, family, and basic human freedom.
...
VPS resources are expensive and often limited. Afrows must avoid waste:
...
- Reseller: representative/mobile-shop seller account. Must be scoped to its own customers, wallet, and orders before a reseller-facing panel is enabled.
...
- Managed admin accounts must store password hashes only. The production/default store is PostgreSQL table `admin_users`; `AFROWS_ADMIN_USERS_FILE` is a legacy local fallback/import source only.
...
- Paid numbers must be write-only in admin APIs and stored as HMAC hashes with `AFROWS_IDENTITY_HASH_KEY` or the deployment secrets key; never return raw paid numbers to the dashboard.
...
- `AFROWS_SECRETS_KEY` must be configured from a deployment secret source before storing Settings/server private keys; encrypted secret rows must expose only references to dashboard clients.
