...
- Payment orders must be idempotent where provider/client keys exist, must store only non-secret provider metadata, and must use audited status transitions. PayPal webhook callbacks must verify PayPal signature headers with the configured webhook ID before any payment state mutation. `client_usage_events` is the append-only usage ledger for consumed traffic and must use source/idempotency keys to avoid double-counting. `payment_order_allocations` is the audited purchase-allocation ledger and must consume each paid order at most once before increasing customer quota. `rewarded_ad_settings` is non-secret policy only; provider secrets, ad-network API keys, and webhook credentials must stay in deployment/encrypted secret storage. `rewarded_ad_grants` is the rewarded-data ledger and must enforce per-client idempotency plus daily caps before increasing account/client quota.
...
- Client subscription refresh endpoints must expose only client-scoped, explicitly published public endpoint metadata, usage multipliers, and secret-safe protocol render readiness. They must not return admin operations, raw outbound config JSON, server credentials, protocol private keys, client UUIDs, PSKs, passwords, certificates, client tokens, paid numbers, or generated secret-bearing config links until a separate audited per-client config renderer exists.
