...
- Telegram bot setup must treat BotFather tokens as write-only secrets: a future superadmin Settings wizard may accept and test the token, but API/UI responses must return only readiness metadata, allowed chat/admin IDs, and last test status, never the token.
