...
- Client subscription refresh endpoints must expose only client-scoped, explicitly published public endpoint metadata and usage multipliers. They must not return admin operations, raw outbound config JSON, server credentials, protocol private keys, client tokens, paid numbers, or generated secret-bearing config links until a separate audited config renderer exists.
