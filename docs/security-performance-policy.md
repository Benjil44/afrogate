...
- Current-panel import preview is read-only and adapter-scoped. It may parse admin-pasted/exported Marzban, X-UI, Sanayi, or generic payloads into normalized candidate summaries, but it must not store raw panel exports, return subscription URLs/tokens, call external panel APIs from the request path, create/update users, or sync quota until a separate audited write flow exists.
