...
- Remote به `Benjil44/afrows` وصل شده و push اولیه انجام شده است.
...
- Frontend: React با Vite و Tailwind CSS برای dashboard سبک، static و سریع در پیاده‌سازی.
- Database: PostgreSQL.
...
Afrows فقط یک API ساده نیست. Backend باید این کارها را داشته باشد:
...
Next.js برای dashboard و full-stack web عالی است، اما backend اصلی Afrows باید worker، metrics ingest، Telegram webhook، alert engine، job scheduling و integration adapter داشته باشد. این‌ها بهتر است در backend مستقل بمانند.

برای شروع low-resource، Vite + React سبک‌تر است و خروجی static ساده‌تری برای Nginx می‌دهد. Tailwind CSS هم سرعت پیاده‌سازی UI را بالا می‌برد و در build خروجی static تولید می‌کند. اگر بعداً server-side rendering لازم شد، می‌توان دوباره Next.js را بررسی کرد.

## جایگاه .NET

.NET/ASP.NET Core انتخاب قوی و enterprise-grade است، مخصوصاً اگر تیم با C# راحت باشد. اما برای شروع Afrows، اگر dashboard با React و bot/integration ها هم TypeScript باشند، NestJS سرعت توسعه و یکپارچگی بیشتری می‌دهد.

اگر بعداً performance یا enterprise مشتری‌ها نیاز خاصی خواستند، می‌توان بعضی سرویس‌ها را با .NET یا Go جدا کرد.

## چرا PostgreSQL؟

داده‌های Afrows رابطه‌ای و audit-sensitive هستند:
...
- usage ledger
- payments/charges
- payment method catalog: PayPal plus manual/card/crypto/bank/local provider keys, with provider secrets kept out of public config; PayPal checkout/capture/webhooks use deployment-secret credentials and verified callbacks
- servers
- tunnels
- route assignments
- alerts
...
MongoDB برای داده‌های document-heavy خوب است، ولی در Afrows billing، audit و relationship ها مهم‌تر هستند.
...
5. ساخت dashboard React/Vite با صفحه overview.
