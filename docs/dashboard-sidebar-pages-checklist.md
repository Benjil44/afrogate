...
- [x] Route canary rollout status with guarded read-only canary guard, rollout, orchestration, and session-safety visibility.
...
- [x] Production server-side protocol apply engine with encrypted secret storage.
...
- [x] Persist admin users in PostgreSQL when production database auth replaces the local file store.
...
- [x] Reseller seller workspace with scoped Dashboard, Users, and Billing sidebar pages, sales charts, Users-page add-user dialog sale action, sold-users table, and wallet selling summary.
