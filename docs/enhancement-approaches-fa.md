...
Afrows باید از اول privacy-first باشد. چون این محصول برای دسترسی امن مردم طراحی می‌شود، نباید داده اضافی جمع کند.
...
- reconciliation job برای اختلاف مصرف بین Afrows و پنل فعلی.
...
- manual/current-panel volume charges should use `quota_charge_events` so support top-ups are audited, optionally idempotent, and explicit about whether an external-panel write was executed.
...
- Card/local gateway checkout preparation should stay generic and secret-safe until a provider-specific signed-request/webhook adapter exists. Bank-transfer and crypto methods can generate payment references and manual instructions, but they must not auto-credit quota before audited admin/provider verification.
...
- rewarded ads should credit small quota grants through admin-managed reward/cap settings and an idempotent, daily-capped ledger; production use should set verification mode to `signed_webhook` or `provider_signed_webhook` and require HMAC-verified ad-network/server callbacks before increasing quota.
...
3. Provisioning abstraction داخل Afrows.
4. Migration tools برای انتقال کاربران.
5. Afrows-native provisioning.

### قانون مهم

هر integration باید پشت adapter باشد تا بعداً تعویض شود، نه اینکه در کل backend پخش شود.

Current implementation note: adapter-based migration now has a read/controlled-import/controlled-usage-sync/local-charge/sanitized-export foundation. `current-panel-import.adapters.ts` normalizes pasted/exported Marzban, X-UI, Sanayi, and generic panel payloads into sanitized import candidates for the Billing page. Admins can then import non-duplicate candidates into Afrows `client_configs`; panel-reported used bytes are recorded as idempotent `panel_sync` baseline usage events instead of raw counter overwrites. Admins can also paste a later export and sync only positive usage deltas for already matched configs; missing, ambiguous, cross-account, duplicate, and non-advancing candidates are skipped. Local charge writes `quota_charge_events`, increases Afrows quota, and returns explicit metadata that no external-panel write ran. Local Afrows client config export returns sanitized summaries only and excludes subscription credentials, secret-bearing config material, raw panel payloads, and tokens. Live external-panel writes, scheduled ongoing reconciliation, and external-panel-native config export remain separate future phases.
