...
Current implementation note: adapter-based migration now has a read-only first step. `current-panel-import.adapters.ts` normalizes pasted/exported Marzban, X-UI, Sanayi, and generic panel payloads into sanitized import candidates for the Billing page. This completes the safe read/preview foundation; controlled writes, usage reconciliation, and config import/export remain separate future phases.
