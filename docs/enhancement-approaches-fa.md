...
Current implementation note: adapter-based migration now has a read/controlled-import/controlled-usage-sync/local-charge/sanitized-export foundation. `current-panel-import.adapters.ts` normalizes pasted/exported Marzban, X-UI, Sanayi, and generic panel payloads into sanitized import candidates for the Billing page. Admins can then import non-duplicate candidates into AfroGate `client_configs`; panel-reported used bytes are recorded as idempotent `panel_sync` baseline usage events instead of raw counter overwrites. Admins can also paste a later export and sync only positive usage deltas for already matched configs; missing, ambiguous, cross-account, duplicate, and non-advancing candidates are skipped. Local charge writes `quota_charge_events`, increases AfroGate quota, and returns explicit metadata that no external-panel write ran. Local AfroGate client config export returns sanitized summaries only and excludes subscription credentials, secret-bearing config material, raw panel payloads, and tokens. Live external-panel writes, scheduled ongoing reconciliation, and external-panel-native config export remain separate future phases.
