...
Current-panel migration starts in the backend billing module with `current-panel-import.adapters.ts` and guarded `POST /api/admin/current-panels/import-preview`. The endpoint is read-only except for an audit event, accepts pasted/exported Marzban, X-UI, Sanayi, or generic payloads, and returns typed shared import-preview contracts without storing raw panel exports or exposing subscription URLs/tokens. The Billing page owns the bilingual preview UI; future write/import/sync flows should reuse the adapter boundary instead of embedding panel-specific parsing throughout services.
