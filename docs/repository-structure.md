...
Afrows uses a small monorepo layout so product, API, dashboard, agent, and deployment assets can evolve together without mixing responsibilities.
...
apps/
  ...
  native-client/ Native VPN client references for OS-only features such as per-app split tunneling
...
agent AFROWS_API_URL -> backend http://127.0.0.1:7000/api
```

The dashboard and client Vite servers use `strictPort` so port conflicts fail fast. Do not rely on Vite fallback ports such as `4001`, `4002`, or `4101`; stop the conflicting process and restart Afrows on the documented port.
...
PostgreSQL runtime access should use the least-privilege `afrows_app` role through `DATABASE_URL`. Migrations should use `afrows_migrator` through `DATABASE_MIGRATION_URL`, while `afrows_owner` remains a no-login ownership boundary for the database/schema.

Database-issued agent tokens are per server and stored only as hashes. Admins can rotate one server's agent token with `POST /api/agents/:serverId/tokens/rotate`; the endpoint revokes active tokens for that server, returns the new plaintext token once, and records an audit event. The legacy `AFROWS_AGENT_TOKEN` remains only a bootstrap/local fallback.

Admin RBAC uses shared permission ids in `packages/shared`, backend guard enforcement through role and permission metadata, and a guarded `/api/admin/permissions` matrix for dashboard visibility. The Users page renders that matrix with localized labels while keeping support/supervisor/auditor sessions away from admin-user management.
...
Agent ping/jitter/packet-loss metrics are collected only for configured synthetic targets via `AFROWS_PING_TARGETS`; empty configuration keeps those fields null. The agent also has opt-in protocol-aware route probes through `AFROWS_TCP_PROBE_TARGETS`, `AFROWS_UDP_PROBE_TARGETS`, `AFROWS_QUIC_PROBE_TARGETS`, `AFROWS_DNS_PROBE_TARGETS`, and `AFROWS_MTU_PROBE_TARGETS`; empty configuration sends no TCP/UDP/QUIC/DNS/MTU route-probe rows. WireGuard route-probe rows are derived from local `wg` telemetry when available, using interface status, active peer count, and handshake freshness without raw keys. MTU probes use configured synthetic hosts and DF/path-MTU checks only; they return safe tunnel MTU recommendations and never inspect user destinations. Route probe rows may include optional `loadedLatencyMs` and `loadedLatencyDeltaMs` values from bounded synthetic checks so the backend can detect paths that look fine at idle but lag under load.
...
Backup status monitoring lives in the backend as a read-only control-plane API under `/api/admin/backups/status`. External backup jobs own backup execution and may publish a compact JSON status file; Afrows returns only sanitized readiness evidence for the dashboard and does not expose local file paths, decrypted data, object-store credentials, raw dumps, or restore controls. Restore readiness lives under guarded `GET /api/admin/backups/restore-plan`, which returns only a non-executable checklist/manual runbook derived from sanitized backup status and keeps production restore execution as a future audited engine.
...
The guarded route health history API, `GET /api/admin/route-health/history`, reads those compact hourly summaries and returns route/outbound/protocol score points for the Routes page. It is read-only, permission-guarded with `routes:read`, and exposes synthetic-probe health only: no user destinations, traffic contents, client IP history, credentials, or data-plane switch actions.
