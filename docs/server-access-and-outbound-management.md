...
Afrows needs server management, traffic visibility, route control, and outbound failover without turning the panel into a vault of reusable root passwords. This document defines the safer shape for the UI and backend.

## Best Practice Summary

- Use a server agent for normal monitoring.
- Use SSH only for bootstrap, repair, and explicit admin actions.
- Prefer SSH keys and a dedicated `afrows` sudo user over reusable root passwords.
...
OpenSSH supports controls such as `PermitRootLogin`, `PasswordAuthentication`, and `AuthorizedKeysFile`; Afrows should generate hardening guidance around these options instead of encouraging permanent root-password use.
...
- `POST /api/admin/servers/:id/credentials` accepts a server credential once, encrypts it with `AFROWS_SECRETS_KEY`, stores it in `server_credentials`, links the active credential id to `server_access_profiles.credential_ref`, and returns only metadata plus the refreshed server summary.
...
The Routes page now has a standalone route canary rollout status surface backed by guarded `GET /api/admin/route-canary/status`. It reuses the preview's current/recommended candidate, rollout plan, rollout evaluation, and orchestration state so operators can see the next action, guard pass/hold state, new-session canary percentage, rollback thresholds, route-consistency hold, and session protection without writing a decision event or moving traffic. While the data-plane adapter is disabled or missing, the status must remain assignment-only/read-only.

Client VPN route preference is now modeled separately from seller/admin operations. A client config can store a coarse detected country, a preferred exit country, or an exact preferred outbound/server through `client_route_preferences`. The backend records only ISO country codes and preference metadata, not client IP history or user destinations. Saving a preference also maintains a per-client route assignment key (`client_config:<id>`) so route decisions can evaluate that client independently from the global default route. Exact outbound/server choice is treated as an explicit lock and should keep sticky-session protection enabled for games and other latency-sensitive sessions. Route decision previews now read that per-client preference context: country mode prefers healthy managed candidates in the preferred exit country when available, exact outbound mode prefers the requested healthy outbound when available, and unavailable preferences fall back to the normal health/session-safe recommendation with explicit reason codes. The first client-scoped API boundary uses admin-issued one-time mobile tokens stored only as hashes in `client_access_tokens`; `/api/client/*` can show the authenticated client's own profile/quota, route preference, selectable non-critical route options, subscription refresh metadata, and can update only that client's route preference when override is allowed. Usage accounting is separate from routing: admin/panel/agent flows record compact `client_usage_events` with idempotency keys, and accepted events update account/client usage counters for remaining-volume reads without storing traffic contents. Outbounds now have `usage_multiplier` from `1x` to `100x`; usage reports tied to a rated outbound store raw bytes, multiplier, and charged bytes so expensive VPS paths can consume quota faster in an auditable way. The client subscription endpoint returns safe public endpoint metadata explicitly published on outbound config, each route's charge multiplier, and protocol-specific render status for WireGuard, VLESS, L2TP, and IKEv2. Connectable config output is rendered only from the authenticated client's own encrypted `client_subscription_credentials` row; missing, invalid, or undecryptable material remains a blocked readiness state instead of leaking route metadata. `apps/client` is the separate mobile-first UX for that boundary: token login, remaining volume, automatic/country/server route choice, subscription server refresh visibility, config render/copy readiness, and score profile selection without admin controls.
...
To keep this efficient on small VPS machines, `RouteQualityAggregationService` compacts recent synthetic probe history into `route_quality_hourly`. The scheduler is enabled by default through `AFROWS_ROUTE_QUALITY_AGGREGATION_ENABLED=true`, runs every `AFROWS_ROUTE_QUALITY_AGGREGATION_INTERVAL_SECONDS`, and looks back `AFROWS_ROUTE_QUALITY_AGGREGATION_LOOKBACK_HOURS`. The analytics endpoint prefers these hourly summaries and falls back to raw metrics if the table is empty or not migrated yet.

The Routes page now has route health score history through `GET /api/admin/route-health/history`. This endpoint reads `route_quality_hourly`, groups by bucket, server, outbound, operator, protocol, and score profile, and returns compact hourly score/latency/jitter/loss points. It is a visibility feature only: it does not inspect user traffic, store user destinations, change route assignments, or apply data-plane routing.

Migration `0008_route_quality_dimensions.sql` expands these summaries by server, outbound key/name, operator, protocol, score profile, day-of-week, and hour-of-day. Agents can optionally tag synthetic route probes with non-secret metadata through `AFROWS_ROUTE_PROBE_ROUTE_GROUP`, `AFROWS_ROUTE_PROBE_OUTBOUND_ID`, `AFROWS_ROUTE_PROBE_OUTBOUND_KEY`, `AFROWS_ROUTE_PROBE_OUTBOUND_NAME`, `AFROWS_ROUTE_PROBE_OPERATOR`, and `AFROWS_ROUTE_PROBE_SCORE_PROFILE`. This supports operator/outbound patterns such as Irelandcell/BTS windows without storing user destinations or inspecting traffic.

Predictive recommendations stay advisory. The backend can flag historically degraded windows within `AFROWS_ROUTE_QUALITY_PREDICTION_LOOKAHEAD_HOURS`, but it does not apply routes automatically; route locks, sticky assignments, cooldown, hysteresis, drain-safe switching, and audit reasons are still required before transparent route changes.
...
The backend runs a lightweight outbound health scheduler when `AFROWS_OUTBOUND_HEALTH_SCHEDULER_ENABLED` is true. It checks only enabled, non-maintenance outbounds whose `last_checked_at` is older than their own `health_interval_seconds`.
...
Current Settings secret storage uses `secret_records` for write-only private-key material. The dashboard sends the secret once, the backend encrypts it with `AFROWS_SECRETS_KEY`, and protocol setup rows store only `secretRef`. The dashboard must not read decrypted secret material back.
...
The live protocol apply boundary now has an audited OpenSSH-based execution engine. Superadmins can submit a live protocol apply request through `/api/admin/settings/protocol-setups/:id/server-apply/live-request`; when any gate is missing, Afrows still records a blocked `protocol_apply_events` row with `dataPlaneMutationExecuted = false`. When every gate passes, the backend records an accepted audit event before mutation, decrypts the protocol secret and SSH private-key credential only in server memory, stages rendered config through `scp`, runs allowlisted remote commands through `ssh`, installs the config, verifies service health, records a final execution snapshot, and never returns stdout, stderr, private keys, protocol secrets, or rendered secret-bearing config to dashboard clients.

Protocol server apply plans now expose the production adapter and command runner while keeping all live mutation disabled by default. A live path must have `AFROWS_PROTOCOL_SERVER_APPLY_ENABLED=true`, `AFROWS_PROTOCOL_SERVER_APPLY_LIVE_EXECUTOR_ENABLED=true`, `AFROWS_PROTOCOL_SERVER_APPLY_SECRET_DECRYPT_ENABLED=true`, and `AFROWS_PROTOCOL_SERVER_APPLY_CREDENTIAL_DECRYPT_ENABLED=true`, plus complete non-secret config material, generated-command policy readiness, a provisioned healthy outbound, an installed server access profile, an active `ssh_private_key` server credential, a protocol `secretRef`, rollback artifacts, audit readiness, and post-apply health verification. WireGuard live rendering requires interface name, address CIDR, listen port, endpoint, allowed IPs, and an actual peer public key. VLESS can render a sing-box inbound from a protocol credential; L2TP/IKEv2 use raw secret config material for the managed single-file apply target. Generated commands remain allowlisted, single-purpose, timeout-bounded, secret-safe, rollback-backed, and free of shell chaining before the runner can execute them.
