...
| Slow egress / low speed / high latency / jitter | `network-performance-engineer` (diagnose per-hop) → `xray-routing-engineer` (egress path) ∥ `anti-censorship-transport-engineer` (entry) → `cto-architect` | if topology changes |
