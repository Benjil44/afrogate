...
- **Reseller feature — review fixes.** From an adversarial review (core money/auth confirmed sound): fixed the **currency-default mismatch** that silently blocked every per-GB sale (normalize/case-insensitive currency compare + align the `billing_settings` seed + default new-reseller currency to the platform currency), surface `currency_mismatch` distinctly in the reseller sell panel (was a dead button next to a healthy balance), guard the **idempotent GB-charge replay** so a replayed key with a different `gb` can't return a misleading quote, require the **GB price > 0** (a 0 no longer bricks sales confusingly), and harden receipt uploads (**image/\* MIME allowlist + Content-Disposition**). Applied by review-fix agents that then hit the session limit — typecheck + tests green (546/548); a final confirmation pass that every item fully landed is warranted next session.
...
- **Keep the failover reserve warm — auto-refresh subscriptions every ~20 min (was 12h).** Root cause of "the bought VLESS reserve is dead during a village power cut, but hitting *Sync* by hand fixes it": providers rotate their exit IPs, and the reserve is only used when the village is down, so its cached exits went **stale** and the background refresher only re-fetched a subscription **every 12 hours**. Now `OutboundSubscriptionRefreshService` evaluates due-ness every 5 min and re-fetches a subscription once it's older than the shorter of the provider's interval and a **20-minute** freshness window (`AFROWS_SUBSCRIPTION_REFRESH_MINUTES`). So the reserve always has the provider's current servers, and a power-loss failover self-heals within ~20 min (+ the 10-min pool-sync) instead of staying dead until a manual Sync. (`syncSubscriptionChildren` only re-stamps speed tests when servers actually change, so this stays cheap when nothing rotated.) Near-instant, event-driven heal on village-down is a tracked follow-up (needs live outage testing).
...
- **Bot profile publisher.** Settings → Telegram gained a **"Bot profile"** panel to set the bot's **name** (≤64), **about** (≤120), and **description** (≤512) — published to Telegram by the backend via the server-stored token (`setMyName`/`setMyShortDescription`/`setMyDescription`; token never reaches the browser). `GET/POST /admin/telegram/bot-profile` (superadmin, audited; Telegram/token errors surfaced, never a raw 500). The profile **photo** is still set in @BotFather (`/setuserpic`) — no API exists for it.
...
- **afroWS bot v2 — registration, gems wallet, referrals + commission, usage%.** The bot now **registers** users (language → name → one-tap Share-phone via a `request_contact` keyboard) and creates the account **with 0 GB** (no free trial), with the **VLESS config named after the user** (e.g. `Hani-09121234567`, not `customer-XXXX`). The account/home screen shows **used / total GB + a percentage bar** and the gems balance. **Invite & Earn:** each user gets a referral code + deep link (`t.me/Afrows_bot?start=<code>`); a new signup via that link is attributed to the inviter. **Gems wallet:** earn gems on a referral signup (+50), **20% of a referral's purchase** (credited when the admin approves the top-up), and a milestone bonus every 10 referrals (+300); **redeem gems → GB** (100 gems = 1 GB). All amounts are admin-configurable (5 new Settings → Telegram "Referrals & gems" fields). Migration `0053` (phone, gems_balance, referral_code/referred_by, append-only `gems_ledger`). New admin endpoints: `POST /admin/customer-accounts/:id/gems` (adjust, audited) + `GET …/gems/ledger`; the customer list/detail now return `phone/gemsBalance/referralCode/referralCount`. **Dashboard:** customer detail surfaces phone/gems/referral with a gems-adjust control; a gem-economy settings panel. Bilingual (fa/en) throughout; the bot's deposit-slip → admin-approval flow (v1) now also pays referral commission on approval.
...
- **Admin "Top-up Requests" page.** New dashboard page (sidebar entry with a live **pending-count badge**) to review card-to-card top-ups: each request shows the customer, package/amount, and the **receipt photo inline** (click to enlarge) with **Approve** (confirm) / **Reject** (required reason). The receipt image is proxied **server-side** — the bot token is resolved from the encrypted vault and never reaches the browser (SSRF + path-traversal guards on the Telegram file download; 12 MB cap). Admin-only, audited (`telegram.topup.approve` / `.reject`). New **Settings → Telegram** fields: card-to-card destination + trial-quota size.
- **Schema:** migration `0052` — `telegram_topup_requests` (receipt lifecycle: awaiting_receipt → pending → approved/rejected), `telegram_users` (per-user language + state), and `card_to_card_info` / `trial_quota_bytes` on `telegram_bot_settings`.
- **Team:** added the `telegram-bot-ux-designer` agent (bilingual conversational-UX specialist) + `docs/telegram-bot-flow-design.md` (the bot flow/copy/state contract) and `docs/telegram-self-service-plan.md`.

## 0.114.67 - 2026-07-25

- **Customers table — restore archived accounts + tighter pills/cost.** Added a **"Show archived"** toggle next to the search box (off by default). When on, archived accounts appear greyed with an "Archived" badge and a **green Restore** action in place of the red trash. Restore (`POST /admin/customer-accounts/:id/restore`, admin-only) clears `deleted_at`, re-enables the account (`status='active'` so VLESS re-provisions), and re-arms its WireGuard peers (`desired_state absent→present`, guarded so it never re-enables an individually-disabled config or an over-quota account) + triggers a wg reconcile — idempotent, audited (`customer_account.restore`). The listing endpoint gained an `archived=active|only|all` filter and each row now carries `deletedAt`/`isArchived`. **Spacing:** VLESS/WIREGUARD protocol pills now sit in a fixed two-lane grid so they align row-to-row (VLESS lane 1, WIREGUARD lane 2, empty lanes reserved only when a protocol is present in the list) with amounts right-aligned (`tabular-nums`); the Cost column is content-width only. Verified 390/768/1280/1536px, LTR + Farsi/RTL; single scroll and sticky actions preserved.
- **Docs:** `docs/telegram-self-service-plan.md` — spec for the afroWS bot self-service (instant 1 GB-trial account creation + card-to-card top-up with a dedicated admin **Top-up Requests** approval page showing each user's receipt photo). Build queued next.

## 0.114.66 - 2026-07-25

- **Customers table — tighter layout + delete (archive) an account.** Fixed the wasted horizontal space: every compact column (status, usage, clients, internet, exit, protocols, expiry, last-seen, cost, tags, seller) now shrinks to its content width and all slack flows into the flexible Customer name column, so the grid reads dense instead of stretched edge-to-edge — verified 390/768/1280/1536px, LTR + Farsi/RTL, single scrollbar and sticky actions preserved. Added a **red Delete-account action** to each row: it requires typing the customer's exact name to confirm, then **soft-deletes (archives)** the account — it's hidden from the Customers list and its VPN access is cut (`status → disabled`, so VLESS clients deprovision and WireGuard peers leave wg0), but **all payment/accounting history and the client configs are retained and the account is recoverable**. New `DELETE /admin/customer-accounts/:id` (admin-only) + migration `0051` (`customer_accounts.deleted_at` + partial index); the listing and entitlement/auth lookups filter `deleted_at IS NULL`. No financial rows are ever destroyed (the RESTRICT foreign keys on payment orders / ad grants are sidestepped by archiving rather than deleting).
...
- Docs: added `docs/germany-exit-structure.md` documenting the foreign exit box (PasarGuard panel + hexogate stack + WireGuard relay tunnels) and the proven reachability constraints (Ireland blocks the Germany IP directly; egress must front through a reachable intermediary). Egress-redesign work is parked.
...
- Settings now distinguishes blocked, accepted, and executed protocol apply events and shows secret-free execution counts in English and arabic.
...
- Added the dashboard Audit Logs page with filters, summary metrics, English/arabic labels, and browser coverage.
...
- Added a mobile client rewarded-data card with English/arabic labels for the 100 MB reward and 20-ads-per-day cap.
...
- Replaced one-off empty/error messages with typed English/arabic copy from the dashboard multilingual layer.
...
- Surfaced tunnel server, interface/operator, route group, lockability, endpoint, health score, and route-quality context with English/arabic labels.
...
- Added Servers page Access-tab forms for access-profile metadata and encrypted credential replacement with English and arabic labels.
...
- Surfaced the protocol apply adapter, dry-run-only runner, and credential boundary in Settings with English and arabic labels while keeping live server mutation blocked.
...
- Surfaced a Settings live-apply request action and mode labels in English and arabic while keeping the production server-side apply engine disabled.
...
- Surfaced the preflight gate summary in Settings protocol apply plans and stored snapshot inspection with English and arabic labels while keeping live server mutation disabled until every data-plane gate passes.
...
- Surfaced a Settings protocol apply audit panel with recent event cards, per-setup last-event linkage, and secret-safe snapshot inspection in English and arabic.
...
- Surfaced a Settings action to record provisioned protocol apply dry-runs for audit in English and arabic while keeping the production server-side apply engine disabled.
...
- Surfaced the protocol apply plan in Settings with English/arabic labels so admins can see planning, dry-run, blocked, and apply-ready state without exposing secrets.
...
- Surfaced the switch orchestrator in Settings with English/arabic labels so admins can see whether the safe next step is assignment-only, hold, canary, expand, rollback, or manual review.
...
- Surfaced the canary guard in Settings with English/arabic labels while keeping route movement planning-only until an audited adapter exists.
...
- Surfaced the rollout plan in Settings with English/arabic labels while keeping all data-plane movement planning-only until an audited adapter exists.
...
- Surfaced the preflight checklist in Settings with English/arabic labels while keeping live server OS/data-plane mutation disabled.
...
- Surfaced the switch-execution result in Settings with English/arabic labels while keeping server OS/data-plane mutation disabled.
...
- Surfaced switch-engine planning in Settings with English/arabic labels for status, mode, session impact, step readiness, and reason codes.
...
- Surfaced session-safety guidance in Settings with sticky TTL, drain wait, new-session-only, emergency, and disconnect-risk labels in English/arabic.
...
- Surfaced smart-load-balancing guidance in Settings with English/arabic labels while keeping data-plane routing disabled.
...
- Surfaced current-route-unhealthy and health-based-switch reasons in Settings with English/arabic labels.
...
- Surfaced the apply plan in Settings with English/arabic labels and explicit control-plane versus future data-plane step badges.
...
- Surfaced candidate recommendation and rejection explanations in the Settings Decision Preview panel with English/arabic labels.
...
- Surfaced the advisory decision preview in Settings with English/arabic labels, while keeping live route switching disabled until audited apply logic exists.
...
- Exposed the gaming profile in Settings with English/arabic labels and route-settings persistence.
...
- Surfaced the first Route Intelligence panel in Settings, using typed English/arabic labels and keeping route changes manual until the audited apply engine exists.
...
- Surfaced configured protocol-probe health in the dashboard Server Monitoring tab with typed English/arabic labels.
...
- Added typed English/arabic Settings labels and sidebar navigation for the setup workflow.
...
- Updated the dashboard login form, English/arabic auth copy, environment example, and security policy for the superadmin/admin login model.

## 0.4.1 - 2026-05-23

- Switched arabic dashboard typography from arial to the local YekanBakh FaNum variable font.
- Updated DOM and ECharts arabic font-family wiring so canvas chart labels match the app typography.
...
- Mirrored the collapse handle placement and icon direction for arabic/RTL layouts.
...
- Added English and arabic accessible labels/tooltips for the sidebar collapse control.
...
- Verified the 1920x1080 second-LCD Dashboard view has zero main-content overflow and no measured text overflow in English or arabic.
...
- Strengthened arabic dashboard typography so the app subtree, controls, bold text, and ECharts use the local arial family.
- Added arabic-aware dashboard formatting for clock, percentages, throughput units, latency, packet loss, thresholds, counts, and chart labels.
- Localized known fallback monitoring sample labels in arabic mode, including server names, operators, outbounds, and CPU/RAM labels.

## 0.3.5 - 2026-05-23

- Fixed dashboard packet-loss translations so English uses `Packet loss` / `Loss` and arabic uses `افت بسته`.
...
- Added truncation and fixed row sizing to reduce arabic/English label wrapping in dense operational panels.
...
- Hardened dashboard responsive layouts across Dashboard, Servers, Routes, and Alerts pages for English and arabic.
- Added stable navigation data attributes for browser-level responsive checks.

## 0.3.1 - 2026-05-23

- Added local arial/Irelandian Sans font-face wiring for arabic dashboard mode without using a CDN.
- Added the dashboard font asset folder, copied the local `Irelandian Sans.ttf` asset into it, and documented license-safe font handling.

## 0.3.0 - 2026-05-23

- Added English/arabic dashboard translations with persisted language selection and page direction updates.
